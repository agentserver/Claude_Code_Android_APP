...
**自洁机制**：HomeFragment.onViewCreated 启动时跑 `cleanupOrphanClaudeJsonl()` 后台线程，扫 `-home-claude/*.jsonl`，比对 SessionStore，删除孤儿（来自旧版 leak）。
...
```
┌─ 上游 AgentServer 服务（agent.cs.ac.cn）派发任务
│
▼
...
```
            ...
            └────────┬────────────────────────┘
                     ...
                     ▼
            ┌─────────────────────────────────┐
            │  /home/claude/.local/bin/claude │  ← 我们的拦截 wrapper
